ActiveAdmin.register CollagePayment do
  ...
  form do |f|
    f.semantic_errors
    f.inputs "Collage payment information" do
  ...
  show :title => proc{|payment| "#{payment.admission_type}, #{payment.study_level} payment fees" } do
    panel "Collage Payment information" do
      attributes_table_for collage_payment do
        row :study_level
        row :admission_type
